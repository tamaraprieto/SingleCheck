...
slurm_stdout=$(sbatch -p cola-corta,thinnodes --array=${num_samples} SingleCheckArray -w $size -a $size $CONFIG $SUFFIX)
echo $slurm_stdout
JOB_id=$(echo $slurm_stdout | awk '{print $4}')
echo "6" $DEPTH $size $JOB_id >> JobIDs.${DATASET}.allsizes.out
done
